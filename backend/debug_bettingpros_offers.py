...
if events_data and events_data.get("events"):
    ...
    # Now let's check if there are props for the Lakers game using the offers endpoint
    if lakers_event_id:
        ...
        for market_id, market_name in prop_markets.items():
            ...
            if offers and offers.get("offers"):
                ...
                # Check for LeBron
                print(f"    Players found: ", end="")
                players_found = []
                for offer in offer_list:
                    label = offer.get("label", "")
                    players_found.append(label)
                    if "LeBron" in label:
                        print(f"\n    -> FOUND LEBRON: {label} {offer.get('line', 'N/A')}")
                print(", ".join(players_found))
            else:
