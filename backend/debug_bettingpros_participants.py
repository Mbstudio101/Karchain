    ...
    for event in events_data["events"]:
        if isinstance(event, dict):
            ...
            if "Lakers" in home_team_name or "Lakers" in away_team_name:
                ...
                # Also print participants info
                if event.get("participants"):
                    print(f"    Participants: {len(event['participants'])}")
                    for p in event["participants"]:
                        print(f"      ID: {p.get('id')}, Name: {p.get('name')}, Abbrev: {p.get('abbreviation', 'N/A')}")
        ...
        for market_id, market_name in prop_markets.items():
            ...
            if offers and offers.get("offers"):
                ...
                if offer_list:
                    ...
                    print(f"    First offer participant ID: {first_offer.get('participant')}")
                    
                    # Try to find participant info
                    participant_id = first_offer.get('participant')
                    if participant_id and event.get("participants"):
                        for p in event["participants"]:
                            if p.get('id') == participant_id:
                                print(f"    Participant {participant_id} maps to: {p.get('name')}")
                                break
